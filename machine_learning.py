...
def train_logistic_regression(features, target, max_iter=1000, penalty='l2', C=1e10, standardization=True,
...
def train_gradient_boost(features, target, loss='log_loss', max_depth=3, learning_rate=0.1, show_train_accuracy=False):
    ...
    fit_model : Fit a classifier.
    """
    gb = GradientBoostingClassifier(loss=loss, max_depth=max_depth, learning_rate=learning_rate,
                                    random_state=alg_random_state)
...
def train_models(feature_sets, target):
    ...
    for feature_set in feature_sets:
        ...
        output_models.append(
            {'name': 'lr', 'features': set_name, 'model': train_logistic_regression(feature_set['features'], target)})
        output_models.append(
            {'name': 'dt', 'features': set_name, 'model': train_decision_tree(feature_set['features'], target)})
        output_models.append(
            {'name': 'rf', 'features': set_name, 'model': train_random_forest(feature_set['features'], target)})
        output_models.append(
            {'name': 'gb', 'features': set_name, 'model': train_gradient_boost(feature_set['features'], target)})
        output_models.append({'name': 'nb', 'features': set_name,
                              'model': train_naive_bayes(feature_set['features'], target, remove_negatives=True)})

    return output_models


def train_stacked_models(initial_models, train_feature_sets, train_target, final_classifier=None, exclude_models=[],
        ...
        if type(clf) is dict:
            ...
            for train_feature_set in train_feature_sets:
                ...
                if train_feature_set['name'] == model['features']:
                    ...
                    predictions[col_name] = \
                        pd.DataFrame(cross_val_predict(clf, scaled_features, train_target, method='predict_proba'))[1]
            ...
            for train_feature_set in train_feature_sets:
                ...
                if train_feature_set['name'] == model['features']:
                    ...
                    predictions[col_name] = \
                        pd.DataFrame(cross_val_predict(clf, train_features, train_target, method='predict_proba'))[1]
...
def test_stacked_models(initial_models, test_feature_sets, test_target, final_model, exclude_models=[],
                        append_features=False, result_row_name=None):
    """
